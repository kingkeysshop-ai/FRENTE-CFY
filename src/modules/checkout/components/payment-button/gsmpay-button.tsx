...
const GSMPayPaymentButton = ({ cart, notReady, "data-testid": dataTestId }: Props) => {
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/gsmpay/create-invoice", {
        method: "POST",
      ...
      if (!res.ok || !data.url) {
        throw new Error(data.error || "No se pudo generar el enlace de pago de GSMPay")
      }
  ...
  return (
    <>
      <button
        ...
        data-testid={dataTestId ?? "gsmpay-payment-button"}
      ...
      <ErrorMessage error={error} data-testid="gsmpay-payment-error" />
    </>
  )
}

export default GSMPayPaymentButton
